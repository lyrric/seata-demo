...
JAVA_GC_OPTS="-XX:+PrintGCDetails -XX:+PrintGCDateStamps -Xloggc:/data/app/logs/tcc-business/gc.log"
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_GC_OPTS -classpath $LIB_JARS com.github.lyrric.Application --server.tomcat.basedir=$TOMCAT_DIR >/dev/null 2>&1 &
echo -e "Check the logs for more details."
PID=`ps -ef | grep 'java' | grep 'com.github.lyrric.tcc.business.BusinessApplication' | grep -v 'grep' | awk '{print $2}'`
